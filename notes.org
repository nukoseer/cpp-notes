...
~copy assignment operator~ which is ~defaulted~ by the compiler must be:
 ...
 #+BEGIN_SRC C++
   ...
   {
   public:
        // The compiler will generate something like this as a copy assignment operator. All non-static data members are assigned by taking their values from other class object's non-static data members. Assignments will be in declaration order.
