...
*** Conversion constructor
 ~Conversion constructors~ can be used for implicit or explicit ~type conversion~. ~Non-class type expressions~ can be converted to a ~class type~  using ~conversion constructors~.
