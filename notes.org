...
*** Function overload resolution
 ...
 If one of the ~viable functions~ takes ~T*~ and the other one takes ~const T*~, this means both of them are ~exact match~. If the argument type is ~const T*~, ~const T*~ will be called, otherwise ~T*~ will be called. This is also valid for references.
 #+BEGIN_SRC C++
   void f1(int*);
   void f1(const int*);
   void f2(int&);
